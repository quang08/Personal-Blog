        ...
        {dataLoaded &&
          categories.map((category, index) => (
            <Link key={index} href={`/category/${category.slug}`}>
              <span className="carousel relative align-middle text-center rounded-full px-3 py-3 text-sm bg-gray-200 dark:text-white dark:bg-neutral-800 text-black transition duration-300 hover:text-blue-400 dark:hover:text-blue-400">
                {category.name}
              </span>
            </Link>
          ))}
